...
// ─── Coin Config ──────────────────────────────────────────────────────────────
// Each coin runs the full BCB strategy independently with its own DCA state file.
// portfolioUSD: budget allocated to this coin (split your total across coins)
...
// Yahoo Finance — Russell 2000 (IWM), Dow Jones (^DJI), VIX (^VIX), IGV ETF
...
// Stochastic RSI — BCB uses this on monthly chart (hits 0 at bottoms)
...
// Detect bearish divergence: price higher high but RSI lower high (distribution signal)
function detectBearishDivergence(weeklyCandles, weeklyCloses) {
  ...
  if (priceHigh <= prevHigh) return false; // no higher high in price
  // rough RSI divergence: compare RSI at those pivot points
...
// High-volume capitulation candle (large body + volume spike)
...
// Spring detection: price briefly dips below prior 30-day low then recovers
...
// 10-day parabola: 100%+ gain in 10 days = BCB's #1 cycle top signal
...
// Wyckoff position based on price structure (works for any coin)
...
async function fetchBTCMacroData() {
  ...
  const [
    ...
    fetchYahooFinance("IWM"),       // Russell 2000 ETF
    fetchYahooFinance("%5EDJI"),    // Dow Jones
    fetchYahooFinance("%5EVIX"),    // VIX Fear Index
    fetchYahooFinance("IGV"),       // iShares Software ETF (BCB's macro trigger)
    fetchFearGreed(),               // Crypto Fear & Greed
  ]);
...
async function detectCoinPhase(btc, symbol) {
  ...
  if (accumScore >= 6 && btc.weeklyRSI !== null && btc.weeklyRSI < 40) {
    ...
    phase = "ACCUMULATION";   // lowered from 3 → 1 to reflect current reality
...
function loadDCAState(dcaFile, legacyMigrate = false) {
  // One-time migration: dca-state.json → dca-state-XRPUSDT.json
  if (legacyMigrate && !existsSync(dcaFile) && existsSync("dca-state.json")) {
    ...
    // Normalise field names (totalXRP → totalCoin)
    const migrated = {
  ...
  // Backwards compat: older files may use totalXRP / xrpPrice
...
async function runCoin(coin, btc, today, now) {
  ...
  // ── DISTRIBUTION — stop buying, alert to sell ────────────────────────────
  ...
  // ── TRANCHE 1 — 25% at capitulation (accumScore 8+) ─────────────────────
...
async function run() {
  ...
  // Check if any coin needs analysis today (skip BTC fetch if all done)
  ...
  // Fetch BTC + macro data ONCE, share across all coins
  const btc = await fetchBTCMacroData();

  // Run each coin sequentially (avoids rate limits)
  for (const coin of COINS) {
    try {
